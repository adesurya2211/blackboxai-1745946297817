...
$no_kk = $nama_kepala_keluarga = $alamat = $rt = $rw = $kelurahan = $kecamatan = $kabupaten = $provinsi = $kode_pos = $anggota_keluarga = "";
$no_kk_err = $nama_kepala_keluarga_err = $alamat_err = "";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($no_kk_err) && empty($no_nik_err) && empty($nama_kepala_keluarga_err) && empty($alamat_err)) {
        $sql = "UPDATE warga SET no_kk = :no_kk, no_nik = :no_nik, nama_kepala_keluarga = :nama_kepala_keluarga, alamat = :alamat, rt = :rt, rw = :rw, kelurahan = :kelurahan, kecamatan = :kecamatan, kabupaten = :kabupaten, provinsi = :provinsi, kode_pos = :kode_pos, anggota_keluarga = :anggota_keluarga WHERE id = :id";
...
<head>
    ...
    <title>Edit Warga - Rukun Tetangga</title>
    ...
    <div class="max-w-3xl mx-auto bg-white p-6 rounded shadow overflow-x-auto">
        ...
        <form action="<?= htmlspecialchars($_SERVER["PHP_SELF"]) . '?id=' . $id ?>" method="post" novalidate>
            ...
            <div class="flex justify-between">
                ...
                <button type="submit" class="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 transition">Update Warga</button>
            </div>
        </form>
    </div>
</body>
